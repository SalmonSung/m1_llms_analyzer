...
`Analyzer` is the only object the notebook needs. It wires the three services
together from one `RunConfig`, owns the loaded model for the session, and
exposes `invoke` / `batch` / `save`. Services stay independently constructible --
...
class Analyzer:
    ...
    def quick(
        ...
        **model_kwargs: Any,
        ...
        return cls(
            RunConfig(
                model=ModelConfig(model_id=model_id, **model_kwargs),
